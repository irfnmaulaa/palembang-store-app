...
class TransactionSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        // 9083
        // 10433
        foreach (Transaction::where('id', '>', 0)->take(3000)->orderBy('id')->get() as $transaction) {
        ...
        foreach (Transaction::where('id', '>', 6043)->take(3000)->orderBy('id')->get() as $transaction) {
            $items = json_decode(json_decode($transaction->items));
            if (!empty($items->data) && is_array($items->data) && count($items->data) > 0) {
                $type = $items->data[0]->method;
        ...
        foreach (Transaction::where('id', '>', 9083)->take(3000)->orderBy('id')->get() as $transaction) {
            $items = json_decode(json_decode($transaction->items));
            if (!empty($items->data) && is_array($items->data) && count($items->data) > 0) {
                $type = $items->data[0]->method;
